#!/usr/bin/env python3
"""
Extract text from a document file for the knowledge-to-notes workflow.
...
Outputs:
  <tempdir>/k2n_work/full_text.txt  — full extracted text
  <tempdir>/k2n_work/metadata.json  — stats and metadata

Set K2N_WORKDIR to override the output directory.
"""
...
OUTPUT_DIR = Path(
    os.environ.get(
        "K2N_WORKDIR",
        str(Path(tempfile.gettempdir()) / "k2n_work"),
    )
)
...
def normalize_install_mode(argv: list[str]) -> str:
    mode = os.environ.get("K2N_INSTALL_MISSING", "no").lower()
    ...
    try:
        with zipfile.ZipFile(epub_path) as zf:
            ...
            if opf_files:
                ...
                spine_order = [str((opf_dir / href).as_posix()) for href in hrefs]
